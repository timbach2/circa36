 ...
 *
 * $Id: new_user.php3,v 1.2 2000/07/12 18:22:32 kk Exp $
 *
...
$T->set_var('body','');
$T->set_var('page_title','User Administration');
	...
	switch ($key) {
		...
		case "u_edit": // Change user parameters
			...
			if (!$perm->have_perm("admin")) {    // user is not admin
				...
				$permlist = addslashes(implode($perms,","));
				$query = "update auth_user set username='$username', password='$password', perms='$permlist' where uid='$u_id'";
				$db->query($query);
				if ($db->affected_rows() == 0) {
					my_error("<b>Failed:</b> $query");
					break;
